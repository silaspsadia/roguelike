...
const STARTING_X: i32 = 40;
const STARTING_Y: i32 = 25;
...
fn new_map() -> Vec<TileType> {
    ...
    for _i in 0..400 {
        ...
        if index != xy_index(STARTING_X, STARTING_Y) {
...
fn try_move_player(delta_x: i32, delta_y: i32, ecs: &mut World) {
    ...
    for (_player, pos) in (&mut players, &mut positions).join() {
        ...
        if map[xy_index(pos.x + delta_x, pos.y + delta_y)] != TileType::Wall {
...
fn main() -> rltk::BError {
    ...
    gs.ecs
        .create_entity()
        .with(Position { x: STARTING_X, y: STARTING_Y })
        .with(Renderable {
            glyph: rltk::to_cp437('@'),
